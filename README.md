...
## Repo Contents
- [data](./data): this folder includes the data required to reproduce the analysis and results. This includes **combined_data.csv** (new infection data for all studies) and **day0_frequencies**. It also includes **Data_dictionary.doc**, with more information on input data.
...
- **R_script_combined_all.R** : this file is the R script which pools and analyses data from all studies using **deterministic_combined_all.stan**. This includes:
  ...
  - Calculation of the mean duration of protection using estimated shape and scale parameters (and median & 95% Credible intervals across 10,000 iterations)
- **model_all_combined.RData**: this file stores the model (MCMC) outputs produced using **R_script_combined_all.R**. Due to size restrictions, this is not uploaded but can be generated by running the **R_script_combined_all.R** file.
- **Figures**: this folder contains code related to figures presented in the main manuscript and supplementary information. These are using both data and outputs from the model and include:
...
- [validation](./validation): this folder contains data and code related to the validation analysis including figure 5 presented in the main manuscript and supplementary tables. This folder contains rstan files for different model structures (including/excluding control group and constant/varying incidence of infection).
